...
export function useBusSchedule(bus) {
  const legs = useRef(buildLegs(bus)).current
  const totalDuration = useRef(legs.reduce((s, l) => s + l.durationSec, 0)).current

  const compute = () => {
    const nowSec = (Date.now() - EPOCH) / 1000 + (bus.phaseOffsetSec || 0)
